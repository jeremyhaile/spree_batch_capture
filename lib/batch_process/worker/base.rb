class BatchProcess::Worker::Base

  
  
  # Actual method called by the Batch Process.
  # takes a BatchProcess object or an id
  ...
  def self.perform(batch)
    ...
    if ! bp || ! bp.kind_of?(Batch)
      raise BatchProcess::Error::InvalidBatch.new "Invalid batch record."
    end
    ...
    if bp.running?
      ...
      raise BatchProcess::Error::InvalidBatch.new "Expected the batch state to be running, got '#{bp.state}' instead." 
    end
