...
spec = eval(File.read('batch_process.gemspec'))
...
task :test_app do
  ...
  class BatchProcesTestAppGenerator < Spree::Generators::TestAppGenerator

    def install_gems
      inside "test_app" do
        run 'bundle exec rake spree_core:install'
        run 'bundle exec rake batch_process:install'
      end
    end
...
gem 'batch_process', :path => \'#{File.dirname(__FILE__)}\'
      gems
    end

  end
  BatchProcesTestAppGenerator.start
end
